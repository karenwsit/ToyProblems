...
class Cell(object):
    ...
    def neighbors(self):
        ...
        return [self.board.cells[self.row + r][self.col + c] for c, r in self.NEIGHBOR_DELTAS
                if 0 <= self.col + c < self.board.width and 0 <= self.row + r < self.board.height]

    def click(self):
        """Click on a cell.

        - If it's a mine, reveal all cells and end the game in loss.
        - Check and reveal the neighbors. If all cells are revealed but mines, end in win.
        """
    ...
    def reveal_and_check_neighbors(self):
        ...
        If a cell is not a mine, reveal it and then do the same for all of its neighbors.
        Repeat until there are no more cells found to check.
        """
        
        while not self.mine:
            for cell in self.neighbors:
                self.show(cell)
                self.reveal_and_check_neighbors(cell)
...
class Game(object):
    ...
    def get_move(self):
        ...
        while True:
            try:
                move = raw_input("Move (col row, like 'ab' - %d left) > " % self.board.left)
